...
function appendAndDelete(s, t, k) {
  ...
  let index = getIndex(s, t);
  if (k >= s.length + t.length) return "Yes";
  let sLength = s.slice(index).length;
  let tLength = t.slice(index).length;
  const remaining = k - sLength - tLength;
  if (remaining >= 0) return remaining % 2 === 0 ? "Yes" : "No";
